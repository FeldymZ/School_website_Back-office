...
const EditProfileModal = ({ me, currentPhotoUrl, onClose, onSaved }: Props) => {
  ...
  const displayedPhoto = photoPreview ?? (!removePhoto ? currentPhotoUrl : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const updated = await UserService.updateMyInfo(
      ...
      setError("Modification impossible (email déjà utilisé ?)");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="relative flex items-center justify-between gap-3 px-4 sm:px-8 py-3.5 sm:py-6 border-b border-gray-100">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1 truncate">Modifier mes informations</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700">
                <UserIcon size={14} className="text-[#00A4E0] flex-shrink-0" />
                Nom
